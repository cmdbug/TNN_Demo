...
//
//    auto proto_path = [[NSBundle mainBundle] pathForResource:@"nanodet_sim_opt.tnnproto" ofType:nil];
//    auto model_path = [[NSBundle mainBundle] pathForResource:@"nanodet_sim_opt.tnnmodel" ofType:nil];
...
NanoDet::NanoDet(bool useGPU) {
    if (NanoDet::net == nullptr) {
        auto modelInfo = loadModelToInfo("nanodet_sim_opt.tnnproto", "nanodet_sim_opt.tnnmodel");
        ...
        NanoDet::device_type = useGPU ? TNN_NS::DEVICE_METAL : TNN_NS::DEVICE_ARM;
...
std::vector<BoxInfo> NanoDet::detect(UIImage *image, float threshold, float nms_threshold) {
    ...
    // 格式转换 RGBA -> BGRA
    cv::Mat dst(image_h, image_w, CV_8UC4);
    cv::Mat src(image_h, image_w, CV_8UC4, imageSource.get());
    ...
    auto input_mat = std::make_shared<TNN_NS::Mat>(dt, TNN_NS::N8UC4, image_dims, dst.data);  // imageSource(RGBA) or dst.data(BGRA)
    // 模型输入
...
//    status = NanoDet::instance->ForwardAsync(callback);
    ...
    for (const auto &head_info : heads_info) {
        ...
        if (status != TNN_NS::TNN_OK) {
            NSLog(@"instance.GetOutputMat Error:cls_pred %s", status.description().c_str());
        }
        ...
        if (status != TNN_NS::TNN_OK) {
            NSLog(@"instance.GetOutputMat Error:dis_pred %s", status.description().c_str());
        }
...
void NanoDet::decode_infer(const std::shared_ptr<TNN_NS::Mat> &cls_pred, const std::shared_ptr<TNN_NS::Mat> &dis_pred, int stride, float threshold, std::vector<std::vector<BoxInfo>> &results) {
...
//            NSLog(@"score: %f  %s", score, labels[cur_label].c_str());
